...
import { getIsNestedTable } from './utils';
...
export function TableNG(props: TableNGProps) {
  ...
  const hasNestedFrames = useMemo(() => getIsNestedTable(data.fields), [data.fields]);
  const tableHasGeoCell = useMemo(() => hasGeoCell(data), [data]);

  const inner = hasNestedFrames ? <TableNested {...props} /> : <TableFlat {...props} />;
